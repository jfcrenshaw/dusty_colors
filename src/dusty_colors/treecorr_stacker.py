...
class TreeCorrStacker:
    ...
    def _setup_randoms(self) -> None:
        """Build random catalogs and weights, delegating to `randoms`."""
        if not self.random_correction:
            return

        catalogs = build_random_catalogs(
            foreground=self.foreground,
            background=self.background,
            footprint=self.footprint,
            foreground_da=self._foreground_da,
            patch_col=self.patch_col,
            treecorr_patch_col=self._treecorr_patch_col,
            use_jackknife=bool(getattr(self, "_use_jackknife", False)),
            multiplier=self.random_multiplier,
            seed=self.random_seed,
            nside=self.random_nside,
            weighting=self.random_weighting,
            colors=self.colors,
        )
        self._random_foreground = catalogs.foreground
        self._random_background = catalogs.background
        self._random_foreground_da = catalogs.foreground_da
        self._random_foreground_weight = catalogs.foreground_weight
        self._random_background_weight = catalogs.background_weight
